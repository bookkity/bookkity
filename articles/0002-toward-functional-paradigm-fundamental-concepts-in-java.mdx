...
On the first look it looks fine, but what in case `calculateActiveReservations()` removes some elements from provided list?
...
If we create list using `List.of()` or `Stream#toList()` it creates `ImmutableCollections.List`, which doesn't allow any modification and informs us about that through a `UnusportedOperationException`.
...
- Behavior seems more like read-only than immutable. Immutable collections should return us a copy.
...
```
As you can guess, we can wrap our method result in generic type.
```scala
...
## Summary
We have learned several key concepts of functional programming. These are the basics that are well-supported by the language and should not disturb our team members.
...
However, looking at Java development, we can observe that it has a tendency to draw from functional programming languages.
