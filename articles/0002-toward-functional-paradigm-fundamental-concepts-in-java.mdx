...
## Introduction
As Java Developers, we are pretty familiar with OOP, because we use object-oriented programming language, but that doesn't mean we can't draw from other paradigms, and can't combine them.
...
In Kotlin, this is the default behavior, we need to explicitly say if the result of the operation is nullable or not.
`?` -> nullable
...
```
This will cause also a change in our in-memory implementation.
```java
class InMemoryFlightLoader implements FlightLoader {
...
```
This seems like nonsense to me, because why the heck should we wrap non-asynchronous code in `CompletableFuture`.
...
```
As you can guess, we can wrap our method result in a generic type.
```scala
...
However, looking at Java development, we can observe that it tends to draw from functional programming languages.
