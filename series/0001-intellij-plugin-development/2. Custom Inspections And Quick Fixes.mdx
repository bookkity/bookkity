...
<img alt="Image description" src="/series/0001/01/example-1.png" width="70%" style={{borderRadius: "18px"}}/>

2. **Mixed annotations**: The developer added two annotations that are not compatible with each other.
<img alt="Image description" src="/series/0001/01/example-2.png" width="70%" style={{borderRadius: "18px"}}/>

3. **Primitive type for the nullable parameter**: The developer used a primitive type for the parameter that can be null.
<img alt="Image description" src="/series/0001/01/example-3.png" width="70%" style={{borderRadius: "18px"}}/>

4. **Invalid value for the annotation**: The developer used an invalid value for the annotation.
<img alt="Image description" src="/series/0001/01/example-4.png" width="70%" style={{borderRadius: "18px"}}/>
...
Imagine that you love the [Optional](https://docs.oracle.com/javase/8/docs/api/java/util/Optional.html) class from Java 8, and you want to use it in your project.
You know that it's a good practice to use `Optional` instead of `null`, but you are tired of writing the same code that wraps the value in `Optional`.
...
class MethodVisitor extends JavaElementVisitor {
    ...
    public void visitMethod(PsiMethod method) {
        ...
        holder.registerProblem(typeElement, "Method uses @Nullable annotation", ProblemHighlightType.WARNING, new UseOptionalQuickFix(method));
    }
}
```

Psi elements are the core of the IntelliJ Platform API. They represent the elements of the code, like classes, methods, fields, etc.
...
public class UseOptionalQuickFix implements LocalQuickFix {
    ...
    public void applyFix(Project project, ProblemDescriptor descriptor) {
        ...
        PsiTypeElement typeElement = method.getReturnTypeElement();
        if (typeElement != null) {
            typeElement.replace(factory.createTypeElementFromText("Optional<" + typeElement.getText() + ">", method));
        }

        // 4. Wrap return statements
        // We wrap the return statements with `Optional.ofNullable` or `Optional.empty` if the return value is `null`.
    ...
    private void importClass(PsiFile file, String classImport) {
        ...
        PsiClass psiClass =  JavaPsiFacade.getInstance(project).findClass(classImport, GlobalSearchScope.allScope(project));
